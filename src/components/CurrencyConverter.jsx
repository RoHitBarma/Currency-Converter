  ...
  return (
    <div className='max-w-xl mx-auto my-10 p-5 bg-white rounded-lg shadow-md'>
      ...
      <div className='flex justify-end mt-6'>
        {convertedRate ? (
          <button onClick={resetCurrency} className='px-5 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition duration-300'>
          Reset
          </button>
        ): (
          <button onClick={convertCurrency} className={`px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:bg-indigo-500 focus:ring-offset-2 cursor-pointer ${converting ? "animate-pulse" : ""}`}>
          Convert
          </button>
        )}
      </div>
